...
def resolve_data_config(
    ...
    assert (
        model or args or pretrained_cfg
    ), "At least one of model, args, or pretrained_cfg required for data config."
    args = args or {}
    ...
    if args.get("input_size", None) is not None:
        ...
        # print(args)
        # CUSTOM
        # Overwrite dataset size
        if "imagenet64" in data_dir:
            input_size = [3, 64, 64]
        if "imagenet32" in data_dir:
            input_size = [3, 32, 32]
        if "imagenet16" in data_dir:
            input_size = [3, 16, 16]
        if "imagenet8" in data_dir:
            input_size = [3, 8, 8]
